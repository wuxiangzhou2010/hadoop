...
int MakeTorrent::make(char * path, char * outFile, char * readPath, const char * track)
{
    using namespace libtorrent;
    strcpy(Path, path);
    strcat(Path, "/\0");
    ...
    file_pool fp;
    std::string full_path = path; // 完全路径
    ...
    if (!strcmp(branch_path(path).c_str(), branch_path(dec_file).c_str() ) ) {
    ...
    if (!strcmp(branch_path(path).c_str(), branch_path(dec_file).c_str() ) ) {
        set_piece_hashes(t, branch_path(full_path), boost::bind(&MakeTorrent::print_progress, _1, t.num_pieces() ), ec); // 向种子加入哈系值
    } else   {
        set_piece_hashes(t, branch_path(dec_file), boost::bind(&MakeTorrent::print_progress, _1, t.num_pieces() ), ec); // 向种子加入哈系值
    }
    ...
    strcpy(Data->FilePath, branch_path(dec_file).c_str() );
...
int MakeTorrent::make(char * readPath, const char * track)
{
    using namespace libtorrent;
    ...
    file_storage fs; // 目标文件
    file_pool fp;
    std::string full_path = readPath; // 完全路径
